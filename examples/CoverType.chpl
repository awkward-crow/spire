/*
  ...
  UCI Cover Type dataset (class 1 Spruce/Fir vs class 2 Lodgepole Pine,
  ~495 k samples, 54 features) and reports log-loss and accuracy on train
  and test sets.

  Workflow:
    1. python save_cover_type.py       # create the CSV (~495 k rows)
    2. make CoverType                  # compile
    3. ./build/CoverType               # run (Chapel)
    4. python lightgbm_cover_type.py   # run (LightGBM baseline)
...
// ------------------------------------------------------------------
proc main() throws {
  writeln("=== Cover Type Classification — Chapel GBM ===");
